...
const RiskIndicator = ({ percentage }) => {
  ...
  const getTextColorClass = () => {
    if (clampedPercentage < 50) return 'text-red-600';
    if (clampedPercentage < 75) return 'text-yellow-600';
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        <span className={`text-lg font-semibold ${getTextColorClass()}`}>
...
const MetricTile = ({ 
  ...
  return (
    <div className={`
      ...
      border rounded-lg p-4 shadow-sm hover:shadow-md transition-all duration-300 relative
    `}>
      {entryNumber !== null && entryNumber > 0 && (
        <div className="absolute top-2 right-2 text-xs font-bold text-gray-400">
          Entry {entryNumber + 1}
        </div>
      )}
